...
from typing import Annotated, TypedDict, List, AsyncGenerator, Dict, Any
...
from data_source import PARTY_DATA, AMENITIES_DATA, get_party_data
...
# Define tools
_SLIM_FIELDS = ('id', 'title', 'location', 'city', 'country', 'time', 'category', 'genre')

def _slim(events: list, limit: int = 30) -> list:
    return [{k: e[k] for k in _SLIM_FIELDS if k in e} for e in events[:limit]]

@tool
def get_party_list():
    """Returns a summary list of up to 30 upcoming parties and events."""
    return _slim(get_party_data())
...
_STATUS_MAP = {
    "get_party_list": "Getting party list...",
    "search_parties": "Searching for parties...",
    "search_parties_by_country": "Searching by country...",
    "get_party_details": "Getting party details...",
    "search_nearby_amenities": "Finding nearby spots...",
    "search_amenities": "Searching amenities...",
    "estimate_travel_time": "Calculating travel time...",
...
async def astream_agent_response(
    ...
    async for event in app_instance.astream_events({"messages": messages}, version="v2"):
        event_type = event["event"]

        if event_type == "on_tool_start":
            tool_name = event.get("name", "")
            yield {"type": "status", "text": _STATUS_MAP.get(tool_name, "Working...")}
    ...
    if not final_result:
        yield {"type": "error", "text": "No response generated."}
        return

    text = final_result.get("response_text", "")
    violation_fix = validate_identity(text)
    if violation_fix:
        text = violation_fix
    ...
    yield {
        "type": "done",
        "recommendations": final_result.get("party_recommendations", []),
        "suggestions": final_result.get("suggested_questions", []),
    }

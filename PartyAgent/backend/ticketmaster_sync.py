...
def _normalize_event(raw: dict) -> dict:
    ...
    # Attractions (performers/artists)
...
# Countries to pull events from for global coverage
_COUNTRY_CODES = [
    "US", "GB", "DE", "AU", "CA",
    "NL", "FR", "ES", "IT", "JP",
    "BR", "MX", "NZ", "BE", "SE",
...
# Keyword searches to capture nightlife/electronic events not in default sort
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        for country in _COUNTRY_CODES:
            for page in range(pages_per_country):
                ...
                # Respect Ticketmaster's 5 req/s rate limit
                await asyncio.sleep(0.25)

        # ── Keyword pass: EDM / DJ / nightlife ──────────────────────────────
